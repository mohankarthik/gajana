...
LOWCONF_REMARK = "REVIEW: low-confidence auto-category"
...
class Categorizer:
    ...
    def categorize(self, txns: list[dict[Hashable, Any]]) -> list[dict[Hashable, Any]]:
        ...
        # Apply fuzzy results (low-confidence -> flag for review).
        for i, cat in fuzzy_pending.items():
            txns[i]["category"] = cat
            _flag_remark(txns[i], LOWCONF_REMARK)
        ...
        if needs_llm and self.llm is not None:
            ...
            for local, gi in enumerate(needs_llm):
                if local in preds:
                    ...
                    _flag_remark(txns[gi], LOWCONF_REMARK)
