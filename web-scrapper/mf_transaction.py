...
class MutualFundTransaction:
  def __init__(self, account: str, date: datetime, scheme: str, folio: str, type: str, units: float, nav: float, value: float, stamp_duty: float, fund_house: str = "", sanitize_name=False) -> None:
    ...
    if not fund_house:
      self.fund_house = [house for house in FUND_HOUSES if self.scheme in house[1]][0][0]
    else:
  ...
  def is_present(self, old_transactions: list[Self]) -> bool:
    if [txn for txn in old_transactions if txn.date == self.date and txn.folio == self.folio and txn.type == self.type and txn.scheme == self.scheme and txn.units == self.units]:
      return True
    return False
